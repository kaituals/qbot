...
class PromoteCommand extends Command_1.Command {
    ...
    async run(ctx) {
        ...
        if (!role.rank || role.rank === 0)
            return ctx.reply({ embeds: [(0, locale_1.getNoRankBelowEmbed)()] });
        ...
        try {
            ...
            ctx.reply({ embeds: [await (0, locale_1.getSuccessfulDemotionEmbed)(robloxUser, role.name)] });
            (0, handleLogging_1.logAction)('Demote', ctx.user, ctx.args['reason'], robloxUser, `${robloxMember.role.name} (${robloxMember.role.rank}) → ${role.name} (${role.rank})`);
        }
        catch (err) {
            console.log(err);
